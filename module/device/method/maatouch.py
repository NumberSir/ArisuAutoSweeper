...
class MaaTouch(Connection):
    ...
    @retry
    def pinch_maatouch(self, p1_1, p1_2, p2_1, p2_2):
        p1 = insert_swipe(p0=p1_1, p3=p1_2)
        p2 = insert_swipe(p0=p2_1, p3=p2_2)
        builder = self.maatouch_builder

        builder.contact = 0
        builder.down(*p1[0]).commit()
        builder.contact = 1
        builder.down(*p2[0]).commit()
        builder.send()
